(() => {
  ...
  function normalizeSnapshot(raw) {
    ...
    return {
      ...
      seedVersions: raw.seedVersions && typeof raw.seedVersions === "object" && !Array.isArray(raw.seedVersions)
        ? Object.fromEntries(Object.entries(raw.seedVersions).map(([key, value]) => [key, String(value)]))
        : {},
  ...
  function normalizeSeeds(raw, ownedCollections) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
    const seeds = {};
    for (const collection of RECORD_COLLECTIONS) {
      const seed = raw[collection];
      if (!ownedCollections.includes(collection) || !seed || typeof seed !== "object") continue;
      const version = String(seed.version || "").trim();
      if (!version) throw new Error(`${collection} seed requires a version`);
      const records = Array.isArray(seed.records) ? deepClone(seed.records) : [];
      if (records.some((record) => !String(record?.id || "").trim())) throw new Error(`${collection} seed records require stable ids`);
      seeds[collection] = { version, mode: seed.mode === "replace" ? "replace" : "merge", records };
    }
    return seeds;
  }

  function applySeeds(snapshot, seeds) {
    const next = normalizeSnapshot(snapshot);
    let changed = false;
    for (const [collection, seed] of Object.entries(seeds)) {
      if (next.seedVersions[collection] === seed.version) continue;
      if (seed.mode === "replace") {
        next[collection] = deepClone(seed.records);
      } else {
        const records = new Map(next[collection].map((record) => [String(record.id), record]));
        seed.records.forEach((record) => records.set(String(record.id), deepClone(record)));
        next[collection] = [...records.values()];
      }
      next.seedVersions[collection] = seed.version;
      changed = true;
    }
  ...
  function createLocalAdapter(options = {}) {
    ...
    const seeds = normalizeSeeds(options.seeds, ownedCollections);
    ...
    return {
      ...
      async load() {
        return enqueue(() => {
          const seeded = applySeeds(readStoredSnapshot(), seeds);
          return seeded.changed ? persistSnapshot(seeded.snapshot) : seeded.snapshot;
        });
      },
      async save(snapshot) {
        return enqueue(() => {
