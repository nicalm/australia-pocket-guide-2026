...
function confirmationEmailUrl(emailId) {
...
function flightCard(journey, index) {
  ...
  return `
    <article class="flight-card" data-journey="${escapeHtml(journey.id)}">
      ...
      ${confirmationEmailLink(journey.confirmationEmailId)}
    </article>
  `;
}
...
function renderStays() {
  const stays = state.data.accommodations || [];
  $("#stay-count").textContent = `${stays.length} STAYS`;
  $("#stay-list").innerHTML = stays.map((stay) => `
    <article class="stay-row">
      <div><span>${escapeHtml(formatCompactDate(stay.checkInDate))} — ${escapeHtml(formatCompactDate(stay.checkOutDate))}</span><h3>${escapeHtml(stay.name)}</h3><p>${escapeHtml(stay.address || stay.city || "")}</p></div>
      <div class="stay-actions">
        ${stay.googleMapsUrl ? `<a href="${escapeHtml(stay.googleMapsUrl)}" target="_blank" rel="noopener">地图 ↗</a>` : ""}
        ${stay.website ? `<a href="${escapeHtml(stay.website)}" target="_blank" rel="noopener">官网 ↗</a>` : ""}
        ${confirmationEmailLink(stay.confirmationEmailId)}
      </div>
    </article>`).join("");
}
...
async function loadExtras() {
  ...
  $("#clothing-list").innerHTML = state.data.days.map((day) => { const plan = outfit[day.date]; return plan ? `<details class="style-day"><summary><span>DAY ${String(day.day).padStart(2, "0")} · ${formatCompactDate(day.date)}</span><strong>${escapeHtml(day.title)}</strong></summary><p><b>穿搭</b> ${escapeHtml(plan.wear)}</p><p><b>拍照</b> ${escapeHtml(plan.photo)}</p><p><b>随身</b> ${escapeHtml(plan.carry)}</p></details>` : ""; }).join("");
  if (!share) { $("#packing-sync").textContent = "使用共享网址后可同步。"; return; }
  ...
  $("#rental-card").innerHTML = `
    <article class="rental-panel">
      ...
      <div class="rental-details">
        ...
        ${rental.document?.url ? `<a class="rental-document-link" href="${escapeHtml(rental.document.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(rental.document.label || "查看租车订单 PDF")} ↗</a>` : ""}
      </div>
    </article>
  `;
...
function createRuntimeAdapters() {
  ...
  const todoSeed = state.data.preTrip?.todoSeed;
  const seeds = localCollections.includes("todos") && todoSeed?.version ? {
    todos: {
      version: todoSeed.version,
      mode: todoSeed.mode,
      records: authoredTodos.map((item, index) => ({
  ...
  const localAdapter = localCollections.length ? storage.createAdapter({ mode: "local", tripId, collections: localCollections, seeds }) : null;
